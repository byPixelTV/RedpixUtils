...
inline fun <reified T : Event> Listener.register(
    ...
    RedpixUtils.instance.server.pluginManager.registerEvent(T::class.java, this, priority, executor, RedpixUtils.instance, ignoreCancelled)
}
...
inline fun <reified T : Event> SingleListener<T>.register() {
    RedpixUtils.instance.server.pluginManager.registerEvent(
        T::class.java,
        this,
        priority,
        { _, event -> (event as? T)?.let { this.onEvent(it) } },
        RedpixUtils.instance,
        ignoreCancelled
    )
}
